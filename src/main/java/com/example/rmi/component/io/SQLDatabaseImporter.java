...
public class SQLDatabaseImporter {
    ...
    private static void importTable(Connection connection, String tableName, int tableIndex) throws SQLException {
        ...
        while (columnResultSet.next()) {
            ...
            if ("MONEYINVL".equals(columnType)) {
                String min = parts[2];
                String max = parts[3];
                DatabaseManager.getInstance().addColumn(tableIndex, columnName, ColumnType.valueOf(columnType), min, max);
            } else {
                DatabaseManager.getInstance().addColumn(tableIndex, columnName, ColumnType.valueOf(columnType));
            }
        }
